...
#line 361 "snl.y"
      {
        TreeNode* thenBlock = newNode(RoutineK);
        thenBlock->kind.routine = RoutineBodyK;
        thenBlock->addChild((yyvsp[-3].node));

        TreeNode* elseBlock = newNode(RoutineK);
        elseBlock->kind.routine = RoutineBodyK;
        elseBlock->addChild((yyvsp[-1].node));

          (yyval.node) = newNode(StmtK);
          (yyval.node)->kind.stmt = IfK;
          (yyval.node)->addChild((yyvsp[-5].node));
        (yyval.node)->addChild(thenBlock);
        (yyval.node)->addChild(elseBlock);
      }
#line 1877 "y.tab.c"
    break;
...
#line 369 "snl.y"
      {
        TreeNode* thenBlock = newNode(RoutineK);
        thenBlock->kind.routine = RoutineBodyK;
        thenBlock->addChild((yyvsp[-1].node));
